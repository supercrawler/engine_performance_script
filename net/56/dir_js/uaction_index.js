...
var play_record={page:'page',getMethod:function(){return(this.page=='play')?'getList':'both';},loginHtml:function(){return usr.gIsLogin()?'':'<span class="log">想永久保存观看记录？请<a href="javascript:play_record.tryLogin();">登陆</a></span>';},sHtml:function(){return'<div class="title" style="cursor: pointer;" onmousedown="play_record.initList();">\
                ...
                <div class="content">\
                    ...
                    <ul class="list" id="rList" onmousedown="setStat(\'ua_openlist\');"></ul>\
                    <p class="opt">'
+this.loginHtml()+'<a href="javascript:play_record.delAll();">清空记录</a><!--<a href="#">更多记录</a>-->\
                    </p>\
...
percent=percent<1?1:percent;var _title=unescape(play_rec.Subject);_st=this.timeToStr(play_rec.playtime,false);var zh_st=this.timeToStr(play_rec.playtime,true);listHtml+='<li id="rcli_ck">\
                                        ...
                                        <p class="i">'+
(percent>90?('已看完<a href="'+play_rec.go_url+'">从头观看</a>'+(play_rec.next_url?'<a href="'+play_rec.next_url+'">下一集</a>':'')):'观看至'+zh_st+'<a href="'+play_rec.go_url+'#'+_st+'" title="已观看'+percent+'%">继续观看</a>')+'</p>\
...
_st=this.timeToStr(data[i].playtime,true);listHtml+='<li id="rcli_'+i+'">\
                                        ...
                                        <p class="i">'+
(data[i].finished==1?('已看完<a href="'+data[i].go_url+'">从头观看</a>'+(data[i].next_url?'<a href="'+data[i].next_url+'">下一集</a>':'')):'观看至'+_st+'<a href="'+data[i].go_url+'" title="已观看'+data[i].percent+'%">继续观看</a>')+'</p>\
                                        <a href="javascript:play_record.delRecord('+i+');" class="close">x</a>\
                                    </li>';}}
rList.innerHTML=listHtml;}
rec.className="sbox record_data box_open";}else{rec.className='sbox record_data box_close';}},callBack:function(record){this.isInitList=true;if(record&&record.data){this.listData=record.data;}
...
return str;},tryLogin:function(){try{login_box();}catch(e){login();}}};maybelike={rows:6,mainID:"maybelike",sHtml:function(){var s='<div class="title2">\
      <span class="side"><a title="更多您可能喜欢的视频" href="javascript:maybelike.load(1);">更多</a></span>\
     ...
     </div>';return s;},load:function(more){more=more||0;var api='http://msg.ua.56.com/api/maybelike.php?more='+more+'&callback=maybelike.loadCallback';if(_.e(this.mainID)){try{if(1||_.getCookie("uaid")){jLoader(api);}}catch(e){}}else{}},loadCallback:function(o){if(o&&o.data&&o.data.length>0){var tpl=this.sHtml();var li_class="",n=1,mc="",loop,d=o.data;loop=tpl.match(/<\{loop\}>(.*?)<\{\/loop\}>/);for(var i in d){if(d.hasOwnProperty(i)&&d[i]){if(n>this.rows){break;}
...
tpl=tpl.replace(loop[0],mc);if(_.e(this.mainID)){_.e(this.mainID).innerHTML=tpl;_.e(this.mainID).className="may_like box_open";_.e(this.mainID).style.display="";}
if(o.uv_stat==1){setStat("ua_like_show");}}else{if(_.e(this.mainID)){_.e(this.mainID).style.display="none";}}},tog:function(){if(_.e(this.mainID).className=="may_like box_open"){_.e(this.mainID).className="may_like box_close";}else{_.e(this.mainID).className="may_like box_open";}}};