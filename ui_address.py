...
def update_engine_en_db(ui_address_animation, version_name):
    connection = sqlite3.connect(mydatabase)
    connection.isolation_level = None
    cursor = connection.cursor()
    str_sql='UPDATE services_dolphinengineen  SET ui_address_animation=%f where version_name="%s"' %(ui_address_animation, version_name)
    print str_sql
    cursor.execute(str_sql)
    cursor.close()

def update_engine_cn_db(ui_address_animation, version_name):
    connection = sqlite3.connect(mydatabase)
    connection.isolation_level = None
    cursor = connection.cursor()
    str_sql='UPDATE services_dolphinenginecn  SET ui_address_animation=%f  where version_name="%s"' %(ui_address_animation, version_name)
    print str_sql
    cursor.execute(str_sql)
    cursor.close()
